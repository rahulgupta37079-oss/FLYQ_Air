...
/**
 * Send Transit Delay Emails to 64 Customers
 * 
 * This script sends tracking emails with delay notification to all customers
 * who have orders with tracking IDs.
 * 
 * Usage: node send-delay-emails.js
 */
...
const DATABASE_URL = process.env.DATABASE_URL || 'file:./.wrangler/state/v3/d1/miniflare-D1DatabaseObject/6d2cdedc73a048e2b1f5a952e3ffb8e0.sqlite';
const FROM_EMAIL = 'FLYQ Drones <noreply@flyqdrones.com>';
const DRY_RUN = process.env.DRY_RUN === 'true'; // Set to 'true' to test without sending

console.log('🚀 Transit Delay Email Sender');
console.log('================================\n');

if (!RESEND_API_KEY && !DRY_RUN) {
  console.error('❌ Error: RESEND_API_KEY environment variable not set');
  console.log('\nUsage:');
  console.log('  RESEND_API_KEY=your_key node send-delay-emails.js');
  console.log('\nOr for dry run (test without sending):');
  console.log('  DRY_RUN=true node send-delay-emails.js');
  process.exit(1);
}
...
// Email template
function generateEmailHTML(order, user) {
  ...
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="font-size: 12px; color: #999; margin-top: 20px;">
        This email was sent to ${user.email} because you placed an order with FLYQ Drones.<br>
...
// Plain text version
function generateEmailText(order, user) {
  const deliveryDay = Math.random() < 0.5 ? 7 : 8;
  const deliveryDate = deliveryDay === 7 ? 'February 7, 2026' : 'February 8, 2026';
  const trackingUrl = `https://flyqdrone.in/track/${order.tracking_id}`;
...
async function sendDelayEmails() {
  try {
    console.log('📊 Fetching orders with tracking IDs...\n');
    
    // Fetch all orders with tracking IDs
    ...
    if (!orders || orders.length === 0) {
      console.log('⚠️  No orders with tracking IDs found.');
      return;
    }
    ...
    if (DRY_RUN) {
      ...
      console.log('Preview of first 3 emails:\n');
      console.log('='.repeat(60));
      
      for (let i = 0; i < Math.min(3, orders.length); i++) {
        const order = orders[i];
        const user = { name: order.customer_name, email: order.customer_email };
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      try {
        const emailHTML = generateEmailHTML(order, user);
        const emailText = generateEmailText(order, user);
        ...
        successCount++;
        console.log(`✅ [${i + 1}/${orders.length}] Sent to ${user.email} (${order.order_number})`);
        ...
        errorCount++;
        console.error(`❌ [${i + 1}/${orders.length}] Failed to send to ${user.email}:`, error.message);
      }
    }
