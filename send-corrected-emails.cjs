...
console.log('📧 Sending Corrected Emails (No Estimated Delivery)...\n');
...
console.log(`Found ${orders.length} orders to email\n`);
...
// Email template WITHOUT estimated delivery
function createCorrectedEmail(order, password) {
...
(async () => {
  try {
    for (let i = 0; i < orders.length; i++) {
      ...
      try {
        await resend.emails.send({
          ...
          html: createCorrectedEmail(order, password)
        });
    ...
    // Save results
    fs.writeFileSync(
      path.join(__dirname, 'corrected-emails-results.json'),
      JSON.stringify(results, null, 2)
    );
    
    console.log('\n' + '='.repeat(60));
    console.log('✅ CORRECTED EMAILS SENT!');
    console.log('='.repeat(60));
    console.log(`✅ Emails Sent: ${results.emailsSent}`);
    console.log(`❌ Failed: ${results.failed.length}`);
    console.log('\n📄 Results saved to: corrected-emails-results.json');
