...
// Get all customer orders (only fields that exist in schema)
...
customers.forEach((row) => {
  ...
  // Insert order item (no updated_at field)
  const product_name = row.product_name.replace(/'/g, "''");
  sqlStatements.push(
    `INSERT OR REPLACE INTO order_items (id, order_id, product_id, product_name, quantity, price, created_at) ` +
    `VALUES (${row.order_item_id}, ${row.order_id}, ${row.product_id}, '${product_name}', ${row.quantity}, ${row.price}, '${row.item_created_at}');`
  );
