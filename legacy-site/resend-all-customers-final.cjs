...
console.log('📊 FINAL Email Campaign - Resending to All 63 Customers\n');
...
async function sendEmails() {
  console.log('📧 Sending welcome emails to all customers...\n');
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    try {
      ...
      console.log(`✅ [${i + 1}/${orders.length}] Sent to ${order.customer_name} (${order.email}) - Message ID: ${result.data?.id}`);
...
(async () => {
  try {
    ...
    // Save results
    const resultsPath = path.join(__dirname, 'final-email-campaign-results.json');
    ...
    console.log(`\n📄 Results saved to: final-email-campaign-results.json`);
    ...
    console.log('\n✨ All emails sent with Message IDs for tracking in Resend dashboard!');
